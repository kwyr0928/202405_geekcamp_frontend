...
setMessage("応援する！")
  ...
  return (
    <div className="flex flex-wrap justify-start items-start bg-cover bg-center p-10">
      <div className={`animate-shadow-drop-center bg-white/75 p-7 rounded-lg relative flex justify-center items-center`}>
        <div className="animate-wobble-hor-bottom">
          <img src={pic} className="relative ml-28 mt-2  animate-wobble-hor-bottom" alt={"爆弾"} style={{ width: '345px', height: '274px' } } />
        ...
        <div className="text-left mt-2 text-base absolute bottom-9 left-9">
          <img className='size-10' src="/src/assets/clear.png" style={{ position: 'absolute', left: '150%'}} alt="" onClick={handleClear} />
          ...
          <div className="flex h-12 justify-left gap-x-6 dark:text-white">
            <a className={`w-64 group flex h-min items-center disabled:opacity-50 disabled:hover:opacity-50 hover:opacity-95 justify-center ring-none rounded-lg shadow-lg font-semibold py-2 px-4 font-dm focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2
             disabled:text-white ring-white text-white border-b-4  active:border-0 hover:text-gray-100 focus-visible:outline-violet-500 text-sm sm:text-base ${color}`} onClick={handleClick}>{message}
            </a>
          </div>
        </div>
      </div>
